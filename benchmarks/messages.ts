...
export class SmallMessage extends Message<SmallMessage> {
    ...
    static sizeOf(msg: SmallMessage): number {
        let s = 0
        if (msg.name !== '') {
            const bl = strLen(msg.name)
            s += 1 + varint32Size(bl) + bl
        }
        if (msg.id !== 0) {
            s += 1 + varint32Size(msg.id)
        }
        if (msg.active) {
            s += 2
        }
    ...
    static encodeTo(msg: SmallMessage, buf: Uint8Array, p: number): number {
        if (msg.name !== '') {
            buf[p++] = 0x0a
            const bl = strLen(msg.name)
            p = writeVarint(bl, buf, p)
            strWrite(msg.name, buf, p, bl)
            p += bl
        }
        if (msg.id !== 0) {
            buf[p++] = 0x10
            p = writeVarint(msg.id, buf, p)
        }
        if (msg.active) {
            buf[p++] = 0x18
            buf[p++] = 1
    ...
    static encode(msg: SmallMessage, w?: BinaryWriter): BinaryWriter {
        ...
        if (msg.id !== 0) {
            w.raw(new Uint8Array([0x10]))
            w.uint32(msg.id)
        }
        if (msg.active) {
            w.raw(new Uint8Array([0x18]))
            w.bool(msg.active)
        }
        return w
    }
...
export class Address extends Message<Address> {
    ...
    static sizeOf(msg: Address): number {
        let s = 0
        if (msg.street !== '') {
            const bl = strLen(msg.street)
            s += 1 + varint32Size(bl) + bl
        }
        if (msg.city !== '') {
            const bl = strLen(msg.city)
            s += 1 + varint32Size(bl) + bl
        }
        if (msg.zip !== '') {
            const bl = strLen(msg.zip)
            s += 1 + varint32Size(bl) + bl
        }
        return s
    }
        ...
        while (r.pos < r.end) {
            ...
            switch (tag >>> 3) {
                ...
                case 3: {
                    const len = r.uint32()
                    const end = r.pos + len
                    msg.address = Address.decodeReader(r, end)
                    break
                }
                case 4:
...
export class LargeMessage extends Message<LargeMessage> {
    ...
    toBinary(): Uint8Array {
        ...
        if (this.payload.length > 0)
            s += 1 + varint32Size(this.payload.length) + this.payload.length
        ...
        if (sbl[0] > 0) {
            buf[p++] = 0x0a
            p = writeVarint(sbl[0], buf, p)
            strWrite(this.title, buf, p, sbl[0])
            p += sbl[0]
        }
        if (sbl[1] > 0) {
            buf[p++] = 0x12
            p = writeVarint(sbl[1], buf, p)
            strWrite(this.description, buf, p, sbl[1])
            p += sbl[1]
        }
        if (sbl[2] > 0) {
            buf[p++] = 0x1a
            p = writeVarint(sbl[2], buf, p)
            strWrite(this.content, buf, p, sbl[2])
            p += sbl[2]
        }
        if (this.version !== 0) {
            buf[p++] = 0x20
            p = writeVarint(this.version, buf, p)
        }
        if (this.score !== 0) {
            buf[p++] = 0x29
            f64Scratch[0] = this.score
            buf[p++] = u8f64[0]!
            buf[p++] = u8f64[1]!
            buf[p++] = u8f64[2]!
            buf[p++] = u8f64[3]!
        ...
        if (this.payload.length > 0) {
            buf[p++] = 0x32
            p = writeVarint(this.payload.length, buf, p)
            buf.set(this.payload, p)
            p += this.payload.length
        }
        for (let i = 0; i < metaLen; i++) {
            const bl = sbl[3 + i]!
            buf[p++] = 0x3a
            p = writeVarint(bl, buf, p)
            strWrite(this.metadata[i]!, buf, p, bl)
            p += bl
        }
        return buf
    }
    ...
    static sizeOf(msg: LargeMessage): number {
        return msg.toBinary().length
    }
    static encodeTo(msg: LargeMessage, buf: Uint8Array, p: number): number {
        const b = msg.toBinary()
        buf.set(b, p)
        return p + b.length
    }
    ...
    static encode(msg: LargeMessage, w?: BinaryWriter): BinaryWriter {
        ...
        if (msg.version !== 0) {
            w.raw(new Uint8Array([0x20]))
            w.uint32(msg.version)
        }
        if (msg.score !== 0) {
            w.raw(new Uint8Array([0x29]))
            w.double(msg.score)
        }
        if (msg.payload.length > 0) {
            w.raw(new Uint8Array([0x32]))
            w.bytes(msg.payload)
        }
        for (const m of msg.metadata) {
            w.raw(new Uint8Array([0x3a]))
            w.string(m)
        }
        return w
    }
        ...
        while (r.pos < r.end) {
            ...
            switch (tag >>> 3) {
                case 1:
                    ...
                    break
                case 2:
                    msg.description = r.string()
                    break
                case 3:
                    msg.content = r.string()
                    break
                case 4:
                    msg.version = r.uint32()
                    break
                case 5:
                    msg.score = r.double()
                    break
